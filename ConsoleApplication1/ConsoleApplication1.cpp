...
vec3 **image = new vec3*[(int)width];	// Creates a pointer array which points to individual values to fill in entries

Shape* shapes[6]; // Pointer to top class in shape creation through virtual with sphere array defined
...
void setup()
{
	...
	// Initialise a loop to run through each pixel of the image, ready for Ray Casting
	for (int j(0); j < height; j++)
	{
		for (int i(0); i < width; i++)
		{
			...
			vec3 rayOrigin(0, 0, 0);						// Ray starts at the camera's world position
			vec3 rayDirection = pCamspace - rayOrigin;		// Define the direction of ray through normal calculation
			rayDirection = normalize(rayDirection);			// Normalize the ray into a unit vector	
